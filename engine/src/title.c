...
static void TitleTerminateObjects(void) {
  int i;

  for (i=0;i<8;i++)
    GoolObjectHandleTraverseTreePostorder(
      (gool_object*)&handles[i], GoolObjectTerminate, 0);
}
...
int TitleUpdate(void *ot) {
  ...
  if (title->transition_state == 5 && fade_counter == 0) { /* currently fading out and fade counter has reached 0? */
    ...
    TitleTerminateObjects(); /* kill all objects spawned in this title state */

...
#define CODEVALUE(v)     ((intptr_t)(v))
#define ISERRORCODE(v)   (CODEVALUE(v) < 0 && CODEVALUE(v) > SUCCESS)
/* can be used to check pointer status, which will have the most significant bit set and also be smaller than -255 */
#define ISSUCCESSCODE(v) (CODEVALUE(v) <= SUCCESS)
#define ISCODE(v)        (CODEVALUE(v) < 0 && CODEVALUE(v) >= -255)
...
#ifndef C1_BOOT_LID
#define C1_BOOT_LID           LID_TITLE
#endif
#define LID_BOOTLEVEL         C1_BOOT_LID

#ifdef C1_BROWSER
_Static_assert(sizeof(void*) == 4, "C1 requires a 32-bit address space");
_Static_assert(sizeof(uint32_t) == 4, "C1 requires 32-bit uint32_t");
#endif

...
test: test-tree
	...
	cc -std=gnu11 -Wall -Wextra -Wno-unused-parameter -Wno-unused-variable -Wno-unused-function -fsanitize=address,undefined -fno-sanitize-recover=undefined -fno-omit-frame-pointer -ffunction-sections -fdata-sections -Iengine/tests/stubs -Iengine/src engine/tests/audio_sampler_test.c engine/src/pc/sound/util.c $(DEAD_CODE_FLAGS) -lm -o /tmp/c1-audio-sampler-test
	ASAN_OPTIONS=halt_on_error=1 UBSAN_OPTIONS=halt_on_error=1 /tmp/c1-audio-sampler-test
...
clean:
	rm -rf "$(DIST_DIR)"
	rm -f /tmp/c1-main-transition-test /tmp/c1-audio-delay-test /tmp/c1-audio-sampler-test /tmp/c1-card-test /tmp/c1-level-ref-test /tmp/c1-gool-null-gop-test \

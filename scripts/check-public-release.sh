...
# a later commit deleted it. The imported spreadsheet is the only binary exception.
prohibited='(^|/)(local-data|streams|dist|node_modules|artifacts|captures|screenshots|recordings|coverage|browser-profile|playwright-report|test-results)(/|$)|(^|/)(\.cache|\.pytest_cache|\.mypy_cache|\.ruff_cache|__pycache__)(/|$)|(^|/)(memory-card\.json|resume\.json)$|(^|/)\.env($|\.)|(^|/)\.DS_Store$|(^|/)id_(rsa|dsa|ecdsa|ed25519)(\.pub)?$|\.(bin|raw|iso|cue|ccd|sub|ecm|chd|cso|img|mdf|mds|nrg|pbp|nsd|nsf|pbak|exe|dll|bios|zip|7z|rar|tar|tar\.gz|tgz|log|tmp|wasm|wat|sav|mcr|srm|state|save|har|webm|mp4|mov|wav|mp3|flac|ogg|p12|pfx|jks|keystore|key|pem)$'
media='\.(png|jpe?g|gif|bmp|tiff?|webp|avif|ico|svg|psd|xcf|blend|fbx|obj|gltf|glb|ttf|otf|woff2?)$'

allowed_placeholder_path() {
  [[ $1 == "engine/streams/README.md" ]]
}

allowed_binary_path() {
  [[ $1 == "engine/doc/memmap.xlsx" ]]
}

expected_binary_hash() {
  case "$1" in
    engine/doc/memmap.xlsx)
...
check_path_stream() {
  ...
  while IFS= read -r -d '' path; do
    ...
    allowed_placeholder_path "$path" && continue
...
while IFS= read -r -d '' path; do
  ...
  if perl -e 'local $/; my $data = <>; exit(index($data, "\0") >= 0 ? 0 : 1)' "$path"; then
    allowed_binary_path "$path" || fail "unapproved binary working-tree file: $path"
    actual_hash=$(sha256sum "$path" | awk '{print $1}')
    expected_hash=$(expected_binary_hash "$path") || fail "binary path has no pinned hash: $path"
    [[ $actual_hash == "$expected_hash" ]] ||
      fail "binary hash differs from its approved import: $path"
